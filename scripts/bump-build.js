...
// Try a few common locations for buildInfo.ts so the script works from different repo layouts
const candidates = [
  path.join(__dirname, '..', 'src', 'renderer', 'buildInfo.ts'),
  path.join(__dirname, '..', 'renderer', 'buildInfo.ts'),
  path.join(__dirname, '..', 'rd-plan', 'renderer', 'buildInfo.ts'),
  path.join(__dirname, '..', 'rd-plan-beta', 'renderer', 'buildInfo.ts'),
  path.join(__dirname, '..', 'src', 'renderer', 'buildInfo.ts'),
];
...
if (!buildInfoPath) {
  console.error('buildInfo.ts not found in expected locations. Tried:', candidates.join(', '));
  process.exit(1);
}
  ...
  try {
    ...
    if (!autoPushDisabled) {
      ...
      if (remotes.length === 0) {
        ...
        // determine current branch - force "dev" branch
