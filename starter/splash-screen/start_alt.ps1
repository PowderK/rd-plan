...
# Script-Scope Referenz auf Fenster für zuverlässigen Zugriff in Event-Handlern
...
$window.Add_Loaded({
    ...
    $timer.Add_Tick({
        ...
        # Prozess prüfen
        $pNameStatic = "RD-Plan"
        $pNameDynamic = [System.IO.Path]::GetFileNameWithoutExtension($appPath)
        
        $proc = Get-Process -Name $pNameStatic -ErrorAction SilentlyContinue
        if (-not $proc) { $proc = Get-Process -Name $pNameDynamic -ErrorAction SilentlyContinue }
        ...
        if ($proc -or $procElectron) {
            ...
            # Prozess-Check-Timer stoppen (über $sender = der Timer selbst)
            # $timer.Stop() würde hier fehlschlagen da $timer aus dem inneren Scope nicht verfügbar
            if ($sender) { $sender.Stop() }
            
            # Einmaligen Verzögerungstimer starten (3 Sekunden)
            # Start-Sleep würde den UI-Thread blockieren - daher DispatcherTimer
            $closeTimer = New-Object System.Windows.Threading.DispatcherTimer
            $closeTimer.Interval = [TimeSpan]::FromSeconds(3)
            $closeTimer.Add_Tick({
                param($s, $ev)
                $s.Stop()  # Einmalig: sofort stoppen
                $win = $script:appWindow
                if ($win) {
                    Write-Log "Splash-Screen wird geschlossen."
                    $win.Close()
                }
            })
            $closeTimer.Start()
